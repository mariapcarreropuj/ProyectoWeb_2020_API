...
class LoginService(val userRepository: UserRepository) {
    ...
    fun login(loginDto: LoginDto): ResponseEntity<*> {
        val encodedPassword = userRepository.findPasswordByUser(loginDto.username)
        return if(encodedPassword != null){
            val statusPassword = BCrypt.checkpw(loginDto.password,encodedPassword);
            ...
            return if (statusPassword)
            {
                ...
                ResponseEntity<TokenDto>(token,
                    ...
                    val message = "the username or password are wrong" 
                ...
                val message = "the username doesn't exist" 
                logger.error(message)
                ResponseEntity<String>(message,
                                    HttpStatus.NOT_FOUND)
            }
